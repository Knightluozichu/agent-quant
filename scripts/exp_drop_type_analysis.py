...
def mom_score(close):
    if len(close) < 11: return 0.0
    return 0.5*(close[-1]-close[-11])/close[-11] + 0.5*(close[-1]-close[-21])/close[-21] if len(close) > 21 else 0.0

def vol20(close):
    if len(close) < 21: return 0.35
    dr = np.diff(close[-21:]) / close[-21:-1]
    return float(np.std(dr) * np.sqrt(252))

def classify(cr, or_, c, o):
    if cr >= DROP_THR: return None
    if or_ < DROP_THR:
...
for code in rq.ETF_POOL:
    ...
    for i in range(61, len(c)):
        ...
        events.append({
            "code": code, "name": rq.ETF_POOL[code], "date": str(df.iloc[i]["trade_date"]),
            "type": label, "ret60": ret60, "mom": mom, "vol20": vol,
            "day_return": cr, "low_open": or_, "intraday": gap,
            "gate_hit": ret60 < RET60_THR and mom > 0,
            "reject_ret60": ret60 >= RET60_THR, "reject_mom": mom <= 0,
        })
...
for feat in feats:
    ...
    for label in ["低开虚跌", "低开真跌", "普通暴跌"]:
        ...
        print(f"    {label:<8} n={len(vals):>3} 均值={np.mean(vals):+.3f} "
...
r60_only = sum(1 for e in low_fake if not e["gate_hit"] and e["reject_ret60"] and not e["reject_mom"])
mom_only = sum(1 for e in low_fake if not e["gate_hit"] and not e["reject_ret60"] and e["reject_mom"])
...
for thr in [0.01, 0.02, 0.03, 0.05, 0.10, 0.20]:
    ...
    print(f"    ret60_thr={thr:+.2f}: 覆盖 {hit2}/{len(low_fake)} = {hit2/len(low_fake)*100:.0f}%")
    if thr == 0.10:
...
for mthr in [0, -0.02, -0.05, -0.10]:
    ...
    print(f"    mom_thr={mthr:+.2f}: 覆盖 {hit3}/{len(low_fake)} = {hit3/len(low_fake)*100:.0f}%")
...
for label in ["低开虚跌", "低开真跌"]:
    ...
    print(f"    {label}: ret60中位数={np.median(ret60s):+.3f} 动量中位数={np.median(moms):+.3f} "
          f"vol20中位数={np.median(vols):.2f}")

r = {"n_events": len(events), "by_type": {}, "analyst_notes": {
