...
class KlineCNN(nn.Module):
    ...
    def __init__(self, n_layers: int = 2, filters: int = 16, dropout: float = 0.3):
        ...
        for _ in range(n_layers):
            convs += [nn.Conv2d(in_ch, f, 3, padding=1),
...
def main() -> None:
    ...
    for code in codes:
        ...
        for t in range(SEQ_N + 1, n - FWD):
            if not np.all(np.isfinite(o["close"][t - SEQ_N:t + 1])):
                continue
            img = render_tech_image(o["close"][:t + 1], o["high"][:t + 1],
    ...
    for lr in GRID_LR:
        for nl in GRID_LAYERS:
            for nf in GRID_FILTERS:
                model, va = train_cnn(imgs[tr_idx], ys[tr_idx], imgs[va_idx], ys[va_idx],
                                      lr, nl, nf, FIX_DROPOUT)
    ...
    lr_b, nl_b, nf_b = (float(best_key.split("_")[0][2:]), int(best_key.split("_")[1][1:]),
                        int(best_key.split("_")[2][1:]))

    # === 4. 最优模型: 训练(含验证) + 测试预测 ===
    model, _ = train_cnn(imgs[tr_idx], ys[tr_idx], imgs[va_idx], ys[va_idx],
                         lr_b, nl_b, nf_b, FIX_DROPOUT)
    # 用全量训练(训练+验证)重新训练
    ...
    print(hdr)
    results = {}
    r_v3 = run_v3_r4_sameday(data, mat_placeholder(data, dates), thr=1.0,
                             start_idx=tb_start)
    results["V3基线"] = {k: r_v3[k] for k in (
        "final_value", "total_return", "ann_return",
        "sharpe", "max_drawdown", "n_trades", "n_events")}
    print(f"  {'V3基线':<20} {r_v3['final_value']:>10,.0f} {r_v3['total_return']:>+9.1%} "
          f"{r_v3['ann_return']:>+8.1%} {r_v3['sharpe']:>6.2f} "
          f"{r_v3['max_drawdown']:>8.1%} {r_v3['n_trades']:>4}")
    r_r4 = run_v3_r4_sameday(data, mat_placeholder(data, dates), thr=0.02,
                             buffer=0.02, start_idx=tb_start)
    results["V3+R4(2.0%_b2%)"] = {k: r_r4[k] for k in (
        "final_value", "total_return", "ann_return",
        "sharpe", "max_drawdown", "n_trades", "n_events")}
    print(f"  {'V3+R4(2.0%_b2%)':<20} {r_r4['final_value']:>10,.0f} "
          f"{r_r4['total_return']:>+9.1%} {r_r4['ann_return']:>+8.1%} "
          f"{r_r4['sharpe']:>6.2f} {r_r4['max_drawdown']:>8.1%} "
          f"{r_r4['n_trades']:>4}")
    for buf in (0.005, 0.01):
        r_cnn = run_dl_backtest(data, mat_placeholder(data, dates), tb_start, None,
                                pmap, buffer=buf)
        name = f"CNN({buf:.1%})"
        results[name] = {k: r_cnn[k] for k in (
            "final_value", "total_return", "ann_return",
            "sharpe", "max_drawdown", "n_trades", "n_events")}
        print(f"  {name:<20} {r_cnn['final_value']:>10,.0f} "
    ...
    # === 7. 年度表现 (CNN 净值曲线分年) ===
    r_cnn_best = run_dl_backtest(data, mat_placeholder(data, dates), tb_start, None,
                                 pmap, buffer=0.005)
    ...
    out = {"meta": {
        ...
        "train": "2019-12~2023-06", "test": "2023-07~2026-08",
        "decision": "每日预测选优, buffer换仓, T日收盘成交(14:50口径)",
    }, "grid": grid, "best": best_key,
      "results": results, "ablation": abl, "yearly_cnn": yearly}
