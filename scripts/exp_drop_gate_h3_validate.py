...
def run_pair(
    data, dates, s0, s1, cost_mult=1.0, thr=None, delta=None, expo=None
) -> tuple[dict, dict]:
...
def main() -> None:
    ...
    print(
        f"  默认: 门控 thr={B_THR} guard=on + 缓冲豁免 + H3 δ={B_DELTA:.0%} {B_ACTION} 降仓{B_EXPO}"
    )
    print("=" * 78)
    ...
    print(
        f"  基线: A={base_a['final_value']:,.0f} B={base_b['final_value']:,.0f} "
    ...
    for name, kw in perturbs:
        ...
        print(
            f"  {name:<22} B/A={ratio * 100:>5.1f}% 回撤={r_b['max_drawdown']:.1%} "
            f"{'✅' if ok else '❌'}"
        )
        results["perturb"][name] = {"ratio": round(ratio, 3), "b_dd": r_b["max_drawdown"], "ok": ok}
    ...
    for mult in (2.0, 3.0):
        ...
        ok = ratio >= 0.85 and dd_ok
        print(
            f"  {mult:.0f}x 成本: A={r_a['final_value']:,.0f} B={r_b['final_value']:,.0f} "
            f"B/A={ratio * 100:.1f}% 回撤 {r_a['max_drawdown']:.1%}→{r_b['max_drawdown']:.1%} "
            f"{'✅' if ok else '❌'}"
        )
        results["cost"][f"{mult:.0f}x"] = {
            "ratio": round(ratio, 3),
            "a_dd": r_a["max_drawdown"],
            "b_dd": r_b["max_drawdown"],
            "ok": ok,
    ...
    print("\n" + "=" * 78)
    print(
        f"  总判定: 滚动{wins}/4 {'✅' if wins >= 3 else '❌'} | "
