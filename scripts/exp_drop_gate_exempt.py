...
from exp_v32_tail_risk import (  # noqa: E402
    IS_END, IS_START, OOS_END, OOS_START, OUTPUT_DIR, WARMUP,
)
...
def select_target_exempt(data: dict, etf_data_at_date: dict, holding: str | None):
    ...
    for code in rq.ETF_POOL:
        ...
        if rq.USE_SHORT_MOM_FILTER and not rq.check_short_momentum(close):
            continue
        if rq.USE_VOL_SPIKE_FILTER and not rq.check_volume_spike(df["volume"].values[:idx + 1].astype(float), close):
            continue
        if rq.USE_DROP_FILTER and not rq.check_single_day_drop(close):
            continue
        if rq.USE_LONG_MOM_FILTER and code in ("513100", "159915") and len(close) > rq.LONG_MOM_PERIOD:
...
def run_pair(data, dates, s0, s1, use_gate: bool, use_exempt: bool, use_h3: bool,
             cost_mult: float = 1.0) -> tuple[dict, dict]:
    """A/B: A=基线, B=门控[+豁免][+H3]."""
    # A: 基线
    rq.check_single_day_drop = ORIG_CHECK
    h3mod.select_target = ORIG_SELECT
    h3mod.H3_ENABLED = False
    r_a = h3mod.run_v3_risk_h3(data, start_idx=s0, end_idx=max(s1 - WARMUP, 0),
    ...
    r_b = h3mod.run_v3_risk_h3(data, start_idx=s0, end_idx=max(s1 - WARMUP, 0),
                               cost_multiplier=cost_mult)
    rq.check_single_day_drop = ORIG_CHECK
    h3mod.select_target = ORIG_SELECT
    h3mod.H3_ENABLED = False
...
def main() -> None:
    ...
    for vname, gate, exempt, h3on in variants:
        ...
        for name, s0, s1 in segs:
            _, r_b = run_pair(data, dates, s0, s1, use_gate=gate,
                              use_exempt=exempt, use_h3=h3on)
