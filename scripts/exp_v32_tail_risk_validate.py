...
def run_pair(
    data, mat, s0, s1, cost_mult=1.0, vol_thr=None, expo=None, dd_flush=None
) -> tuple[dict, dict]:
    ...
    if dd_flush is not None:
        t.DD_FLUSH = dd_flush
    try:
        r_a = run_v3_r4_sameday(
            data, mat, thr=1.0, start_idx=s0, end_idx=max(s1 - WARMUP, 0), cost_multiplier=cost_mult
        )
        r_b = t.run_v3_risk(
...
def main() -> None:
    ...
    for name, kw in perturbs:
        ...
        print(
            f"  {name:<16} B/A={ratio * 100:>5.1f}% 回撤={r_b['max_drawdown']:.1%} "
            f"{'✅' if ok else '❌'}"
        )
    ...
    print("\n" + "=" * 74)
    print(
        f"  总判定: 滚动{n_win}/4 {'✅' if n_win >= 3 else '❌'} | "
