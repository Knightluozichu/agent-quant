...
def run(data, check_fn, exempt, h3on, delta, expo,
        start_idx=0, end_idx=None, cost_multiplier=1.0):
...
def run_new(data, start_idx=0, end_idx=None, cost_multiplier=1.0):
    ...
    return run(data, make_gated(B_THR), B_EXEMPT, True,
               B_H3_DELTA, B_H3_EXPO, start_idx, end_idx, cost_multiplier)
...
def main():
    print("=" * 80)
    print(f"  门控最终验证 | thr={B_THR} drop={B_DROP_THR:.0%} "
    ...
    for seed in range(12):
        r = run(data, make_random_gated(0.5, seed), B_EXEMPT, True,
                B_H3_DELTA, B_H3_EXPO)
        rand_finals.append(r["final_value"])
        print(f"    种子{seed:>2}: 期末 {r['final_value']:>10,.0f} "
    ...
    print(f"    真实机制: {real_final:,.0f} | 随机分布 P90={np.percentile(arr,90):,.0f} "
          f"中位数={np.median(arr):,.0f} | 真实位于 {pct:.0f}% 分位")
    perm_ok = pct >= 95
    print(f"    {'✅ 显著' if perm_ok else '❌ 未显著'}")
    perm_result = {"real": real_final, "randoms": [round(v,0) for v in rand_finals],
                   "percentile": pct, "passed": perm_ok}
    ...
    for name, kw in perturbs:
        r = run(data, make_gated(kw.get("thr", B_THR)),
                B_EXEMPT, True,
                kw.get("delta", B_H3_DELTA), kw.get("expo", B_H3_EXPO))
        ...
        print(f"    {name:<22} B/A={ratio*100:>5.1f}% 回撤={r['max_drawdown']:.1%} "
              f"{'✅' if ok else '❌'}")
    ...
    for name, s0, s1 in (("W1 2020H2-2021", *seg("2020-06-01", "2021-12-31")),
        ...
        wins += int(beat)
        rolling.append({"seg": name, "a": r_a["final_value"], "b": r_b["final_value"],
                        "ratio": round(ratio, 3), "a_dd": r_a["max_drawdown"],
                        "b_dd": r_b["max_drawdown"], "beat": beat})
    ...
    # === 总判定 ===
    print("\n" + "=" * 80)
    checks = [("置换测试", perm_ok), ("参数扰动", pert_ok),
              ("成本压力", cost_ok), ("滚动窗口", roll_ok)]
    ...
    out = {
        "params": {"thr": B_THR, "drop_thr": B_DROP_THR, "drop_lb": B_DROP_LB,
                   "exempt": B_EXEMPT, "h3_delta": B_H3_DELTA, "h3_expo": B_H3_EXPO},
        "base_final": base_final, "new_final": real_final,
        "new_diff": round(real_final / base_final - 1, 4),
        "permutation": perm_result, "perturbation": pert_results,
        "cost": cost_results, "rolling": rolling,
        "checks": {k: v for k, v in checks}, "passed": passed,
    }
