...
SEGMENTS = [
    {"name": "WF-1", "tr0": "2020-01-02", "tr1": "2021-06-30",
     "te0": "2021-07-05", "te1": "2022-06-30"},
    {"name": "WF-2", "tr0": "2020-01-02", "tr1": "2022-06-30",
     "te0": "2022-07-05", "te1": "2023-06-30"},
    {"name": "WF-3", "tr0": "2020-01-02", "tr1": "2023-06-30",
     "te0": "2023-07-05", "te1": "2024-06-30"},
    {"name": "WF-4", "tr0": "2020-01-02", "tr1": "2024-06-30",
     "te0": "2024-07-05", "te1": "2025-06-30"},
]
...
def run_variant(data: dict, thr: float | None, exempt: bool, h3on: bool) -> dict:
    ...
    rq.check_single_day_drop = (
        h3.make_gated(thr, True) if thr is not None else h3.ORIG_CHECK)
...
def main() -> None:
    ...
    for seg in SEGMENTS:
        print(f"\n  [{seg['name']}] train {seg['tr0']}~{seg['tr1']} "
              f"→ test {seg['te0']}~{seg['te1']}")
        ...
        for thr in RET60_GRID:
            ...
            print(f"    train thr={thr:+.2f}: 期末 {rep['final_value']:>10,.0f} "
                  f"夏普 {rep['sharpe']:.2f}")
        ...
        print(f"    test 新方案: 期末 {rep_test['final_value']:>10,.0f} "
              f"夏普 {rep_test['sharpe']:.2f}")
        print(f"    test 基线  : 期末 {rep_base['final_value']:>10,.0f} "
        ...
        results.append({
            "segment": seg["name"], "train_best_thr": float(train_best),
    ...
    print(f"    PBO = {pbo:.1%} ({n_overfit}/{n_total}) "
          f"→ {'过拟合风险低' if pbo < 0.5 else '过拟合风险高'}")

    out = {
        "segments": results, "pbo": round(float(pbo), 3),
