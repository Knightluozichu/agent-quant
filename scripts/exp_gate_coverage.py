...
def main() -> None:
    ...
    for td, holding, target in log:
        ...
        events.append({
            ...
            "gate_hit": hit, "rank": rank, "score": round(float(sc), 4),
            "fwd5": fwd.get(5), "fwd10": fwd.get(10), "fwd20": fwd.get(20),
        })
    ...
    for hz in HORIZONS:
        ...
        print(f"  [{hz}日] 误杀(反弹) {n_fake}/{len(v)} = {n_fake / len(v) * 100:.0f}% "
              f"| 平均 {avg:+.2%}")
    ...
    out = {"n_events": total, "events": events,
