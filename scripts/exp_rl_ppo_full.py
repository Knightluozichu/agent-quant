...
def main() -> None:
    ...
    env2_tr = ETFEnv(data, dates, mat, feats_map, price_map, codes, tr_s2, tr_e2,
                     initial_capital=f1)
    ac2 = ppo_train(env2_tr, N_EPISODES, DEVICE)
    env2_te = ETFEnv(data, dates, mat, feats_map, price_map, codes, te_s2, te_e2,
                     initial_capital=f1)
    ...
    v3_full = {k: r_v3_full[k] for k in ("final_value", "total_return", "ann_return",
    ...
    v3_seg = {k: r_v3_seg[k] for k in ("final_value", "total_return", "ann_return",
                                       "sharpe", "max_drawdown", "n_trades")}
    ...
    for name, r in (("V3(同区间)", v3_seg),
                    ("RL+CNN(拼接)", {"final_value": f2, "total_return": rl_total,
                                      "sharpe": round((r1["sharpe"] + r2["sharpe"]) / 2, 3),
                                      "max_drawdown": max(r1["max_drawdown"],
                                                          r2["max_drawdown"])})):
    ...
    print("=" * 74)
    print(f"  V3全量: 期末 {v3_full['final_value']:>10,.0f} ({v3_full['total_return']:+.1%}) "
    ...
    out = {"meta": {
        ...
        "device": DEVICE, "ppo_episodes": N_EPISODES,
        "rl_span": "2022-07-01~2026-08-03", "v3_full_span": "2019-12-05~2026-08-03",
    }, "rl_seg1": r1, "rl_seg2": r2,
      "rl_combined": {"final_value": f2, "total_return": rl_total},
      "v3_full": v3_full, "v3_same_span": v3_seg, "beat": beat}
