...
def build_features(mat: dict, vol_mat: dict, dates: list) -> tuple[np.ndarray, list]:
    ...
    for code in codes:
        ...
        for t in range(WARMUP, n - FWD):
            ...
            feats = {}
            for w, k in ((3, "mom3"), (5, "mom5"), (10, "mom10"),
                         (20, "mom20"), (60, "mom60")):
            ...
            feats["dn_streak"] = float(_streak(np.diff(close[t - 5:t + 1]) / close[t - 5:t], -1))
            ...
            meta.append({"date": str(dates[t]), "code": code,
                         "y5": float(y5), "y_up": float(y5 > 0), "y_dn": float(y5 < 0)})
...
def make_models() -> dict[str, object]:
    ...
    models["GBDT(HistGB)"] = HistGradientBoostingClassifier(
        max_depth=4, l2_regularization=1.0, max_iter=300,
...
def importance_report(model, feat_names: list, top_k: int = 10) -> list[dict]:
    ...
    return [{"feature": feat_names[i], "importance": round(float(imp[i]), 5)}
...
def main() -> None:
    ...
    results = {"meta": {
        "period": f"{dates[0]} ~ {dates[-1]}", "n_samples": len(meta),
        "features": feat_names, "fwd_days": FWD,
    ...
    for fold, (tr_idx, te_idx) in enumerate(tscv.split(x), 1):
        ...
        for name in models:
            r = split["models"][name]
            print(f"    {name:<15} 涨AUC {r['up']['auc']:.3f} (acc {r['up']['accuracy']:.1%})  "
    ...
    for name in models:
        ...
        print(f"  {name:<15} {up_auc:>8.3f} {dn_auc:>8.3f} "
    ...
    for task, y in (("涨", y_up), ("跌", y_dn)):
        ...
        if feats_rank:
            ...
            imp_summary[task] = [{"feature": k, "importance": round(avg_imp[k], 5)}
                                 for k in order]
    ...
    if "涨" in imp_summary and "跌" in imp_summary:
        ...
        diff = sorted(all_f, key=lambda f: abs(up_rank.get(f, 20) - dn_rank.get(f, 20)),
                     reverse=True)[:6]
        for f in diff:
            up_r = up_rank.get(f)
            dn_r = dn_rank.get(f)
