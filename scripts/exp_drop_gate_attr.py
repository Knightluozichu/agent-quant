...
def run_is(gated: bool) -> tuple[dict, list[dict]]:
    ...
    report = run_v3_risk(
        data, start_idx=max(a - WARMUP, 0), end_idx=max(min(b, len(dates)) - WARMUP, 0)
    )
    log = DECISIONS
...
def diff_equity_curve(eq_base, eq_gate, label: str) -> None:
    ...
    merged = eb["equity"].rename("base").to_frame().join(eg["equity"].rename("gate"), how="outer")
    print(f"\n  【{label} 净值对比 (月末)】")
    prev_gap = 0.0
    for d, row in merged.iterrows():
        ...
        print(f"    {d.strftime('%Y-%m')}  基线{b:>11,.0f} 门控{g:>11,.0f} 差 {gap:+.1%}{marker}")
        prev_gap = gap
...
def main() -> None:
            ...
            div_points.append(
                {
                    "date": dg["date"],
                    "holding": dg["holding"],
                    "base_target": db["target"],
                    "gate_target": dg["target"],
                    "base_top3": db["top3"],
                    "gate_top3": dg["top3"],
                    "base_n": db["n_cand"],
                    "gate_n": dg["n_cand"],
    ...
    for p in div_points[:40]:
        print(
            f"    {p['date']} 持{p['holding']}: 基线→{p['base_target']} "
            f"(候选{p['base_top3']}) | 门控→{p['gate_target']} (候选{p['gate_top3']})"
        )

    # 分歧点后20交易日收益差 (近似分歧影响)
    data = rq.load_data()
    common = sorted(set.intersection(*[set(data[c]["trade_date"]) for c in list(data.keys())]))
    ...
    for p in impacts:
        if p["impact"] is not None:
            total_diff += p["impact"]
            print(
                f"    {p['date']} 基{p['base_target']}({p['fwd20_base']:+.1%}) "
    ...
    result = {
        ...
        "gate_final": rep_gate["final_value"],
        "n_divergence": len(div_points),
        "divergence": impacts,
    }
